...
2. git commit   
   将文件从暂存区添加到仓库(当前分支)，此时用`git diff -cached`来比较暂存区和参考之间的差异

### 逆向操作
1. git reset <HEAD_log>
   将HEAD指针指向指定的HEAD_log
   - git reset --soft   
     - 仅重置HEAD指针而不会破坏任何东西(工作区和暂存区均保持原样)
     - 若reset之前在工作区对文件做过修改，此时`git diff --cached`为空，`git diff`能显示差异
     - 若reset之前在工作区对文件无修改，此时`git diff --cached`能显示差异，`git diff`为空
   - git reset --mixed
     - 将从尚未提交的暂存区域还原这些更改。仅从暂存区域恢复更改。对文件的工作副本进行的实际更改不受影响
     - 若reset之前在工作区对文件无修改，此时`git diff --cached`为空，`git diff`能显示差异
     - 若reset之前在工作区对文件做过修改，此时`git diff --cached`为空，`git diff`能显示差异
   - git reset --hard
     - 将HEAD指针重置为特定提交ID的最新提交，并删除本地文件更改
